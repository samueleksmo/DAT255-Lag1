...
function xmlMessagesToJs(xml, portcallid, callback) {
    ...
    var result = [];
    var str = '</ns2:portCallMessage>';
    var portcallLength = str.length;
    var funktion = function (err, message) {
    ...
    while (xmlSlice !== "") {
        parseString(xmlSlice, funktion);

        xmlSlice = xmlSlice.slice(xmlSlice.indexOf('</ns2:portCallMessage>') + portcallLength);
